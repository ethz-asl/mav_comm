...
/**
 * \brief Computes the MAV state (position, velocity, attitude, angular velocity) from the flat state.
 *        Additionally, computes the acceleration expressed in body coordinates, i.e. the acceleration
 *        measured by the IMU. No air-drag is assumed here.
 *
 * \param[in] acceleration Acceleration of the MAV, expressed in world coordinates.
 ...
 * \param[out] acceleration_body Acceleration expressed in body coordinates, i.e. the acceleration usually
 *                               by the IMU.
 * \param[out] angular_velocity_body Angular velocity of the MAV, expressed in body coordinates.
 */
...
inline void EigenMavStateFromEigenTrajectoryPoint(const Eigen::Vector3d& acceleration, const Eigen::Vector3d& jerk,
                                         double yaw, double yaw_rate, Eigen::Quaterniond* orientation,
                                         Eigen::Vector3d* acceleration_body,
                                         Eigen::Vector3d* angular_velocity_body) {
  EigenMavStateFromEigenTrajectoryPoint(acceleration, jerk, yaw, yaw_rate, kGravity, orientation, acceleration_body,
                               angular_velocity_body);
}

/// Convenience function with EigenTrajectoryPoint as input and EigenMavState as output.
inline void EigenMavStateFromEigenTrajectoryPoint(const EigenTrajectoryPoint& flat_state, double magnitude_of_gravity,
                                         EigenMavState* mav_state) {
  assert(mav_state != NULL);
  EigenMavStateFromEigenTrajectoryPoint(flat_state.acceleration_W, flat_state.jerk_W, flat_state.getYaw(), flat_state.getYawRate(),
...
/**
 * \brief Convenience function with EigenTrajectoryPoint as input and EigenMavState as output
 *        with default value for the magnitude of the gravity vector.
 */
inline void EigenMavStateFromEigenTrajectoryPoint(const EigenTrajectoryPoint& flat_state, EigenMavState* mav_state) {
